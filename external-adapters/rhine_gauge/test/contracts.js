...
describe("Oracle and OracleClient for Rhine Gauge", function () {
  ...
  it("oracle should emit the correct event when the client is called to request data from the oracle", async function() {
    ...
    const date = p[1].value;
    console.log(`  ... > data request with id ${data_req_id} for date ${date} created in oracle`);
    assert.strictEqual(date, yesterday, `date = ${date} is not yesterday = ${yesterday}`);
    const initiator = p[2].value;
    assert(str_upper_eq(initiator, client_sc.address, `initiator is wrong: ${initiator}`));
  });
  ...
  it("should send the data to the oracle which should send it to the client, emit the correct events and finally have data stored in client's state", async function() {
    ...
    const log = tx_rec.event_logs[0];
    console.log('  ... > event emitted by the client when receiving data in its callback');
    console.log(log);
    const name = log._eventname;
    assert.strictEqual(name, 'callback_data', `_eventname is wrong ${name}`);
    const p = log.params;
    assert.strictEqual(p[0].value, data_test_value.toString(), `data in event is wrong: ${p[0].value}`);
    assert.strictEqual(p[1].value, yesterday, `date in event is wrong: ${p[1].value}`);
    ...
    const value = state.all_data[yesterday];
