...
[..]/unixtime/$ yarn$ curl -X POST -H "content-type:application/json" "http://localhost:8080/" --data '{ "id": 0, "data": { "reqID": 0} }'
...
const customParams = {
  reqID: ['requestID', 'rID'] // the id assigned by the oracle contract for this current request
}
...
const createRequest = (input, callback) => {
  ...
  const oracle_address = bc_setup.addresses.Oracle;
  ...
  Requester.request(config, customError)
    ...
    .then( (state) => {
//      console.log(" ====> state of oracle contract after having called set_data(.) is: ")
//      console.log(state)
      console.log(` ====> in oracle state: value field of entry in DataRequest map for request with id = ${config.params.reqID}`)
      console.log(state.data_requests[config.params.reqID].arguments[2])
    })
    .then( )
    .catch(error => {
